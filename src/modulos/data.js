...
function get_usuario_by_id__promise(id) {
    return new Promise((resolve, reject) => {
        connection.query(`SELECT * FROM usuarios WHERE id = '${id}';`, (err, res, fields) => {
            if (err) reject(err);
            resolve(res[0])
        })
    })
...
function add_usuario__promise(data) {
    return new Promise((resolve, reject) => {
        connection.query(`CALL add_usuario('${data.id}','${data.name}', '${data.username}', '${data.mail}', '${data.password}');`, (err, res, fields) => {
            if (err) {
                reject(err)
            } else {
                console.log(res)
...
function add_grupo__promise(data) {
    return new Promise((resolve, reject) => {
        connection.query(`CALL add_grupo('${data.id}','${data.name}', '${data.inv_code}');`, (err, res, fields) => {
            if (err) {
                reject(err)
            } else {
                if (res !== undefined) {
                    resolve(res[0][0])
                } else { throw new Error("No se pudo crear el grupo") }
            }
        })
    })
}